...
export async function create(server: McpServer): Promise<void> {
  if ((await exists("/data")) && !(await exists("/data/MEMORY.md"))) {
    await writeFile("/data/MEMORY.md", "No memory yet.");
  }
...
function runPipeline(
  ...
  if (first !== undefined && commands.length === 1) return runExec(first.program, first.args);
